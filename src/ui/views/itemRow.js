...
// One row of a list: the title, a tag control, and the action button(s) that fit the item's status.
// Pure construction: it never touches the store, and every interaction goes out through `handlers`:
//   { tag(id, q), startRetag(id), push(id), recall(id), resolve(id), reopen(id), focus(id),
//     hover(id | null), menu(x, y, item) }
...
// opts = { retagging: boolean, fresh: boolean, handlers }
export function buildRow(item, { retagging, fresh, handlers }) {
