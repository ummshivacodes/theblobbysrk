...
function createWindow() {
  ...
  if (process.env.THREAD_AXIS_SELFTEST === '1') {
    ...
    win.webContents.once('did-finish-load', () => {
      ...
      setTimeout(() => { log('expanded'); run("addTask('selftest thread'); tagTask(state.threads.at(-1).id, 1); dispatchToAxis(state.threads.at(-1).id); toggleFocus(state.threads.at(-1).id)"); }, 1600);
      setTimeout(() => { run("(() => { const id = 'selftest-recall'; state.threads.push({id, text: 'recall check', quad: 2, status: 'dump', createdAt: Date.now()}); dispatchToAxis(id); const onAxis = state.threads.find(x => x.id === id).status; recallToDump(id); const backInDump = state.threads.find(x => x.id === id).status; deleteTask(id); console.log('[selftest-status] push/recall:', onAxis, '->', backInDump); })()"); }, 1700);
      setTimeout(() => { log('expanded+task'); run("resolveThread(state.threads.at(-1).id)"); }, 2400);
